...
const PricingCard = ({ plan, popular = false }: { plan: any, popular?: boolean }) => {
    const { t } = useLanguage();
    return (
        <div className={`border-2 ${popular ? 'border-accent' : 'border-gray-700'} bg-[#1c1c1e] p-8 flex flex-col relative`}>
            {popular && (
            ...
            <h3 className="text-4xl font-bold text-white uppercase tracking-wider">{t(plan.title)}</h3>
            <p className="mt-4 text-gray-400 flex-grow">{t(plan.description)}</p>
            <div className="mt-6">
                <span className="text-6xl font-black text-white">{t(plan.price)}</span>
            ...
            <NavLink
                to="/contact"
                className={`mt-10 block w-full text-center font-bold py-3 px-8 text-lg uppercase tracking-wider rounded-sm transition-colors duration-300 transform hover:scale-105 ${popular ? 'bg-accent text-black hover:bg-accent-dark' : 'bg-gray-700 text-white hover:bg-gray-600'}`}
            >
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-32">
            <div className="text-center mb-16">
                <h1 className="text-7xl font-extrabold uppercase tracking-wider text-white">{t('membership.title')}</h1>
