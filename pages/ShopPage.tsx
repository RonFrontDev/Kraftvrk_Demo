...
const ProductCard = ({ name, price, image }: { name: string, price: string, image: string }) => (
    <div className="bg-white dark:bg-[#1c1c1e] overflow-hidden group shadow-lg rounded-md">
        <div className="relative">
            <img src={image} alt={name} className="w-full h-80 object-cover" />
            <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors duration-300"></div>
        </div>
        <div className="p-6">
            <h3 className="text-3xl font-bold text-gray-900 dark:text-white uppercase">{name}</h3>
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-32">
            <div className="text-center mb-12">
                <h1 className="text-7xl font-extrabold uppercase tracking-wider text-gray-900 dark:text-white">{t('shop.title')}</h1>
