...
const Calendar = ({ currentMonth, onMonthChange, selectedDate, onDateSelect, pinnedDates, t }: CalendarProps): React.ReactNode => {
  ...
  const renderHeader = () => {
    ...
    return (
      <div className="flex items-center justify-between pb-4">
        ...
        <h3 className="text-2xl font-bold text-white uppercase tracking-wider">{monthName} {year}</h3>
        <button
          onClick={() => onMonthChange(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))}
          className="p-2 rounded-full hover:bg-gray-700 transition-colors"
  ...
  const renderDays = () => {
    ...
    return (
      <div className="grid grid-cols-7 text-center font-semibold text-gray-400 text-sm uppercase">
        {days.map(day => (
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        if (!isCurrentMonth) {
            ...
            dayClasses += "text-gray-200 hover:bg-gray-800";
        }
