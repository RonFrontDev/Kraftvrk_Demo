        ...
        <div key={index} className="space-y-2 border-t border-gray-700 pt-6 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            <div>
                ...
                <textarea id={`beginner-${index}`} rows={3} value={section.beginnerScaling} onChange={(e) => handleSectionChange(index, 'beginnerScaling', e.target.value)} className={`${inputClasses} border-green-800 focus:border-green-500 focus:ring-green-500`} placeholder={t('wod.form.scalingPlaceholder')}></textarea>
            </div>
            <div>
                <label htmlFor={`advanced-${index}`} className={`${labelClasses} text-red-400`}>{t('wod.form.advancedScaling')}</label>
                <textarea id={`advanced-${index}`} rows={3} value={section.advancedScaling} onChange={(e) => handleSectionChange(index, 'advancedScaling', e.target.value)} className={`${inputClasses} border-red-800 focus:border-red-500 focus:ring-red-500`} placeholder={t('wod.form.scalingPlaceholder')}></textarea>
            </div>
          </div>
        </div>
      ))}
